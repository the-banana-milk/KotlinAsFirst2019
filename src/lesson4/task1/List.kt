...
fun factorize(n: Int): List<Int> {
    ...
    var root = sqrt(remN.toDouble())
    while (minDiv.toDouble() <= root) {
        if (remN % minDiv == 0) {
            need.add(minDiv)
            remN /= minDiv
            root = sqrt(remN.toDouble())
        }else minDiv += 2
    }
...
fun translate(partOfN: Int, thousand: Boolean): List<String> {
    ...
    if ((partOfN % 10 in 1..9) && (partOfN % 100 / 10 != 1)) {
        if (thousand && (partOfN % 10 == 1)) list.add("одна")
        else if (thousand && (partOfN % 10 == 2)) list.add("две")
